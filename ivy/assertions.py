...
def check_greater(x1, x2, allow_equal=False, message=""):
    ...
    if allow_equal and ivy.any(x1 < x2):
        raise ivy.exceptions.IvyException(
            f"{x1} must be greater than or equal to {x2}"
            if message == ""
            else message
        )
...
def check_elem_in_list(elem, list, message=""):
    message = message if message != "" else f"{elem} must be one of {list}"
    if elem not in list:
...
def check_shape(x1, x2, message=""):
    message = (
        message
        if message != ""
        else f"{x1} and {x2} must have the same shape ({ivy.shape(x1)} vs {ivy.shape(x2)})"
    )
...
def check_gather_input_valid(params, indices, axis, batch_dims):
    ...
    if params.shape[:batch_dims] != indices.shape[:batch_dims]:
        raise ivy.exceptions.IvyException(
            f"batch dimensions must match in `params` and `indices`; saw {params.shape[:batch_dims]} vs. {indices.shape[:batch_dims]}"
        )
...
def check_gather_nd_input_valid(params, indices, batch_dims):
    ...
    if indices.shape[-1] > (len(params.shape[batch_dims:])):
        raise ivy.exceptions.IvyException(
            f"index innermost dimension length must be <= rank(`params[batch_dims:]`); saw: {indices.shape[-1]} vs. {len(params.shape[batch_dims:])} ."
        )


def check_one_way_broadcastable(x1, x2):
    return not any(b != 1 and a != b for a, b in zip(x1[::-1], x2[::-1]))


def check_inplace_sizes_valid(var, data):
    if not check_one_way_broadcastable(var.shape, data.shape):
        raise ivy.exceptions.IvyException(
            f"Could not output values of shape {data.shape} into array with shape {var.shape}."
        )
